...
import { SHAPE_DEFAULTS, TREE_DEFAULTS, getPartitionNodeDimensions } from "./tree-defaults";
import type { BorderStyleValue, ExternalNodeShape, PartitionNodeType } from "./tree-defaults";

/** One step in a node's attribute path, e.g. { attribute: "Price Tier", value: "premium" }. */
export type PartitionPathStep = {
  attribute: string;
  value: string;
};

/**
 * The backend partition-tree node object, as needed by the visualization
 * Workflow Modal to filter SKUs by `path` and to break the tree at a level.
 */
export type WorkflowNodeObject = {
  id: string;
  parent_id?: string | null;
  node_name?: string;
  type?: "root" | "attribute" | "value" | string;
  level?: number | null;
  branch?: string | null;
  path?: PartitionPathStep[];
