...
/**
 * The preview-rollup endpoints return loosely-shaped envelopes whose id and
 * payload fields can live at the top level or under `data`/`result`. We probe
 * all of them defensively, so narrow to a permissive shape at the call site.
 */
type LoosePreviewEnvelope = {
  status?: string;
  task_id?: string;
  polling_url?: string;
  preview_id?: string;
...
export function useBaseTestingMergePreview({ caseId, partitionId, canSubmit }: Params) {
  ...
  const submitPreview = React.useCallback(
    async ({ attributeKey, attributeName, nodeId, groups, groupLabels }: SubmitArgs) => {
      ...
      try {
        const postRes = (await WorkflowApi.previewRollup(
          caseId,
          partitionId,
          req,
        )) as LoosePreviewEnvelope;
