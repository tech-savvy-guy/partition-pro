...
import { Warning, View, Close, Download } from "./icons";
...
import { UploadErrorDisplay } from "./upload-error-alert";
  ...
  useEffect(() => {
    const fetchPartitionDatasets = async () => {
      if (caseId && partitionId) {
        try {
          const response = await PartitionApi.getPartitionDatasets(caseId, partitionId);
          if (response.datasets) {
            setDatasets(response.datasets as unknown as Dataset[]);
          }
        } catch (error) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      notify("Upload successful", "success", {
        description: "Grouping data uploaded successfully.",
      });
      onClose?.();
    } catch (err: any) {
      setUploadStatus("error");
          ...
          <DataGrid<Dataset>
            ...
            columns={[
              ...
              },
              {
                ...
                field: "created_by",
                header: "Created By",
                cell: (rowData) => rowData.created_by,
              },
              {
                key: "created_on",
                field: "created_on",
                header: "Created",
                cell: (rowData) =>
                  new Date(rowData.created_on).toLocaleDateString(),
              },
              {
                key: "actions",
                header: "Actions",
