...
import {
  ...
  serializeTreeState,
} from "@/lib/partition-tree/tree.types"
import type {
  ExternalNodeData,
...
function extractPartitionGraph(payload: any): PartitionGraph | null {
  const graph = payload?.partition_tree ?? payload?.data?.partition_tree ?? null
...
function PartitionTreeInner({
  ...
  const applyLayoutDefaults = useCallback(
    (nextLayout: TreeLayoutDefaults) => {
      ...
      const updatedPartitionNodes = newPartitionNodes.map((n) => {
        ...
        const nodeType = (n.data as PartitionTreeNodeData)?.nodeType ?? "value"
  ...
  const handleNodeClick = useCallback(
    (e: React.MouseEvent, node: Node) => {
      ...
      if (node.type === "external") {
        ...
        if (!additive) {
          setSelectedExternalNodeIds((prev) => (prev.length === 0 ? prev : []))
        }
      }
    },
    [editMode]
  ...
  const onDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (
        !shape ||
        !["circle", "box", "diamond", "label", "callout"].includes(shape)
      )
        return
      ...
      const newNode: Node<ExternalNodeData> = {
        ...
        data: {
          ...
          customColor:
            shape === "label" || shape === "callout" ? "#333333" : "#ffffff",
  ...
  const startObmStatusPolling = useCallback(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (statusNorm === "completed" || statusNorm === "failed") {
          setObmProcessing(false)
          stopObmStatusPolling()
          if (statusNorm === "completed") {
            // The recompute replaced the workflow's `obm` result — refresh the
            // OBM tab's cached roi/latest payload so it reflects the new tree.
            void queryClient.invalidateQueries({
              queryKey: ["roi-latest", caseId, partitionId],
            })
          }
          return
        }
  ...
  const loadAttributeSelectionData = useCallback(
    async (nodeObj: any, token?: number) => {
      ...
      try {
        ...
        setAttributeSelectionData(resHasData ? (res as any).data?.data : res)
    ...
    (nodeId: string) => {
      ...
      const pollForResults = async () => {
        ...
        try {
          ...
          if (pollTokenRef.current !== token) return
          const statusValue = String(pollResponse?.status ?? "").toUpperCase()
          const terminal =
  ...
  React.useEffect(() => {
    let mergedLayout = treeDefaultsRef.current.layout ?? TREE_DEFAULTS.layout
    ...
    if (caseId && partitionId) {
      ...
      if (persisted) {
        ...
        partitionNodes = partitionNodes.map((n) => {
          ...
          if (!ov) return n
          const width = typeof ov.width === "number" ? ov.width : n.style?.width
          const height =
          ...
          <div className="flex min-w-0 items-center gap-2">
            {isEditingPartitionTitle && !readOnly ? (
              <input
                ...
                onBlur={() => {
                  const next = partitionTitleDraft.trim() || "Partition Title"
                ...
                className="w-full max-w-[520px] rounded border border-gray-300 px-2 py-1 text-2xl font-semibold text-gray-900 focus:ring-2 focus:ring-red-500 focus:outline-none"
                autoFocus
              ...
              <h2
                ...
                title={readOnly ? partitionTitle : "Double-click to edit title"}
              >
                {partitionTitle}
              </h2>
            )}
  ...
  return (
    <div
      ...
      className={`partition-tree-editor flex min-h-0 w-full flex-col bg-white ${
    ...
    >
      ...
      <TreeDefaultsProvider value={treeDefaults}>
        {isFullScreen ? (
          ...
          <div className="mx-4 mt-3 mb-3 flex flex-row items-stretch overflow-hidden rounded-lg border border-gray-200 bg-white">
      ...
      >
        <DialogContent
          ...
          className="w-[92vw] max-w-[800px] gap-0 rounded-xl bg-gray-50 sm:max-w-[800px]"
        >
