...
--
-- TOC entry 329 (class 1255 OID 116421)
-- Name: get_basemath_matrix_rows_by_skuname(uuid, uuid, text[]); Type: FUNCTION; Schema: core; Owner: -
--

CREATE FUNCTION core.get_basemath_matrix_rows_by_skuname(_case_id uuid, _pp_basemath_id uuid, _selected_skunames text[] DEFAULT NULL::text[]) RETURNS TABLE(sku_l text, sku_r text, roi real)
    ...
    WHERE pss.case_id = _case_id
      ...
      -- LEFT side restricted to node SKUs
      AND pss.data->>'skuname_ean' IN (
            SELECT sku FROM selected_skus
      )

      -- RIGHT side restricted to node SKUs
      AND kv.key IN (
            SELECT sku FROM selected_skus
      );
$$;
