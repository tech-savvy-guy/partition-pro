...
import type { BaseTestingItem } from "./BaseTestingResult";
import {
...
export default function BaseTestingResultWithAttribute({
  ...
  const [editableRows, setEditableRows] = React.useState<any[]>(() =>
  ...
  React.useEffect(() => {
    setEditableRows(
      parsed.tableRows.map((r) =>
        flattenRow(r, parsed.colLabels, parsed.shape),
      ),
    );
  }, [parsed.tableRows, parsed.colLabels, parsed.shape]);
  React.useEffect(() => {
    setColumnOrder(parsed.colLabels.map((_, idx) => idx));
  }, [parsed.colLabels]);
  const baseRowIndexByKey = React.useMemo(() => {
    const map = new Map<string, number>();
    parsed.tableRows.forEach((row, idx) => {
      map.set(String(row?.__rowKey ?? ""), idx);
    });
    return map;
  }, [parsed.tableRows]);

  const handleCellEditComplete = React.useCallback((e: any) => {
    setEditableRows((prev) =>
      prev.map((row) =>
        row.__rowKey === e.rowData.__rowKey
          ? { ...row, [e.field]: e.newValue }
          : row,
      ),
    );
  }, []);
  const handleRowReorder = React.useCallback((e: any) => {
    if (!Array.isArray(e?.value)) return;

    const nextRows = [...e.value];
    ...
    nextRows.forEach((row: any) => {
      ...
      if (
        ...
        baseIdx >= 0 &&
        baseIdx < parsed.colLabels.length &&
        !seen.has(baseIdx)
      ) {
    ...
    // Preserve any unmatched columns (for non-square matrices).
    parsed.colLabels.forEach((_, idx) => {
      if (!seen.has(idx)) nextOrder.push(idx);
    });

    if (nextOrder.length === parsed.colLabels.length) {
      setColumnOrder(nextOrder);
    }
  }, [baseRowIndexByKey, parsed.colLabels]);
  ...
  return (
    <div className="w-full min-w-0 p-4 h-full flex flex-col min-h-0">
      ...
      <div ref={tableContainerRef} className="flex-1 min-h-0">
        ...
        <Column
          rowReorder
          headerStyle={{ width: "2.5rem", flexShrink: 0 }}
          bodyClassName="text-center cursor-grab text-gray-400"
        />
        ...
        <Column
          ...
          header={
            <EditableHeader
              ...
              onChange={(v) => setHeaderLabels((prev) => ({ ...prev, rowStatus: v }))}
            />
          }
          body={(row: any) => {
        ...
        <Column
          ...
          header={
            <EditableHeader
              ...
              onChange={(v) => setHeaderLabels((prev) => ({ ...prev, nRow: v }))}
            />
          }
          body={(row: any) => row.nRow ?? ""}
          headerClassName="col-center"
          bodyClassName="col-center"
          headerStyle={{ width: "90px" }}
        />

        {/* Attribute value — header editable, cells editable (editor prop present) */}
        <Column
          field="rowLabel"
          header={
            <EditableHeader
              value={headerLabels.rowLabel ?? (parsed.attribute || "Attribute value")}
              onChange={(v) => setHeaderLabels((prev) => ({ ...prev, rowLabel: v }))}
            />
          }
          bodyClassName="base-testing-attribute-value-cell"
          ...
          editor={(options) => (
            <input
              ...
              autoFocus
              className="w-full border-0 outline-none bg-transparent text-xs p-0.5"
            />
          )}
          body={(row: any) => {
            ...
            <Column
              ...
              header={
                <EditableHeader
                  ...
                  onChange={(v) => setHeaderLabels((prev) => ({ ...prev, [field]: v }))}
                />
              }
              ...
              body={(row: any) => {
                const raw = row[field];
                const styleObj = getBaseTestingHeatmapStyle(raw, effectiveMeta);
                ...
                return (
                  <div
                    ...
                    title={txt}
                  >
                    <span className="base-testing-heatmap-cell-text">{txt}</span>
                  </div>
                );
              }}
            />
